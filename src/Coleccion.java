...
/*************************************************************************************/
                long duracion= cursos.stream()
                .filter(x->x.getDuracion()>5).count();
                System.out.println("Cantidad de cursos con una duración mayor a 5 horas: "+duracion);
/*************************************************************************************/
                long duracion2=cursos.stream()
                .filter(x->x.getDuracion()<2).count();
                System.out.println("Cantidad de cursos con una duración menor a 2 horas: "+duracion2);
/*************************************************************************************/
        List<String> titulos = cursos.stream()
                .filter(c->c.getVideos()>50)
                .map(c->c.getTitulo()).collect(Collectors.toList());
        System.out.println("Títulos de cursos con cantidad de videos mayor a 50: "+titulos);
/*************************************************************************************/



        double totl_horas= cursos.stream()
                .mapToDouble(Curso::getDuracion)
                .reduce(0,(a,b)->a+b);
        System.out.println("El total de Horas es: "+totl_horas);

/*************************************************************************************/
        double duracion_prom = (totl_horas/(cursos.stream().count()));
...
/*************************************************************************************/
        List<String> mayores_promedio2 = cursos.stream().filter(c->c.getDuracion()>duracion_prom).map(c->c.getTitulo()).collect(Collectors.toList());
        System.out.println("Títulos de los 3 cursos con mayor duración: "+mayores_promedio2);


/*************************************************************************************/
        List<String> alumnos500 = cursos.stream()
                .filter(c->c.getAlumnos()<500)
                .map(c->c.getTitulo())
                .collect(Collectors.toList());
        System.out.println("Títulos de cursos con menos de 500 alumnos: "+alumnos500);



/*************************************************************************************/
        double duracion_max = cursos.stream()
                .mapToDouble(Curso::getDuracion)
                .max().getAsDouble();
        List<String> curso_max = cursos.stream()
                .filter(c->c.getDuracion()==duracion_max)
                .map(c->c.getTitulo()).collect(Collectors.toList());
        System.out.println("El curso de mayor duración es: "+curso_max);

/*************************************************************************************/
        List<String> titulos2= cursos.stream().map(x->x.getTitulo()).collect(Collectors.toList());
        System.out.println("Todos los cursos son: ");
        for (String l:titulos2)
        {
